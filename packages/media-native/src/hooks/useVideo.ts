...
export interface UseVideoResult { video: Video | null; loading: boolean; error: MediaError | Error | null; }
...
export function useVideo(id: number | null | undefined): UseVideoResult {
  ...
  useEffect(() => {
    if (id === null || id === undefined) { setVideo(null); setLoading(false); setError(null); return; }
    let cancelled = false;
    setLoading(true); setError(null);
    client.getVideo(id)
      .then((result) => { if (cancelled) return; setVideo(result); })
